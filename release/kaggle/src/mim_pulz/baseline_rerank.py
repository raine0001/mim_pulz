...
class DualTfidfLexRerankTranslator:
    """
    Dual TF-IDF retrieval (char+word) to get candidates,
    then rerank candidates using lexical memory keywords inferred from source.
    """

    def __init__(self, top_k: int = 50, alpha_char: float = 0.65, alpha_word: float = 0.35):
        self.top_k = top_k
        ...
        self.lex = LexicalMemory(min_src_count=3, top_en_per_src=6)
    ...
    def fit(
        ...
        self.lex.fit(self.train_src, self.train_tgt)
    ...
    def predict(self, test_src: list[str], top_k: int | None = None) -> list[str]:
        ...
        for q in test_src:
            ...
            keywords = set(self.lex.predict_en_keywords(qn, max_keywords=30))

            # If lexical memory yields nothing, fall back to best cosine
            if not keywords:
                preds.append(self.train_tgt[int(cand[0])])
                continue
            ...
            for idx in cand:
                ...
                # keyword coverage in candidate translation
                tgt = self.train_tgt[int(idx)].lower()
                hit = sum(1 for kw in keywords if kw in tgt)
                cov = hit / max(1, len(keywords))

                # length sanity: prefer similar src lengths
                src_len = max(1, len(self.train_src[int(idx)]))
                len_ratio = min(q_len, src_len) / max(q_len, src_len)

                score = base + 0.60 * cov + 0.25 * len_ratio
                if score > best_score:
                    best_score = score
                    best_idx = int(idx)
